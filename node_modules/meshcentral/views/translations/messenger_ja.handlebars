...
<body style="font-family:Arial,Helvetica,sans-serif">
    <div id="xtop" style="position:absolute;left:0;right:0;top:0;height:38px;background-color:#036;color:#EEE;box-shadow:3px 3px 10px gray">
        ...
        <div style="position:absolute;background-color:#036;right:0;height:38px">
            <div id="saveButton" class="icon15 topButton" style="margin-right:4px" title="会話を保存" onclick="saveChatSession()"></div>
            <div id="notifyButton" class="icon13 topButton" style="display:none" title="ブラウザ通知を有効にする" onclick="enableNotificationsButtonClick()"></div>
            ...
            <div id="recordIcon" class="deskareaicon" title="サーバーはこのセッションを記録しています" style="background-color:red;margin:6px;margin-top:7px;border-radius:12px;height:24px;width:24px;float:right;display:none"></div>
        </div>
    ...
    <div id="xbottom" style="position:absolute;left:0;right:0;bottom:0px;height:36px;background-color:#036;font-size:18px">
        <table style="width:100%">
            <tbody><tr>
                ...
                <td style="width:1px">
                    <input type="button" id="sendButton" value="送る" style="box-sizing: border-box;float:right;font-size:18px" onclick="xsend(event)">
                </td>
                <td style="width:1px">
                    <input type="button" id="clearButton" value="クリア" style="box-sizing: border-box;float:right;font-size:18px" onclick="displayClear()">
                </td>
            </tr>
        </tbody></table>
    </div>
    <div id="remoteVideo" style="position:absolute;right:24px;top:45px;width:320px;height:calc(240px + 30px);background-color:gray;border-radius:12px 12px 12px 12px;box-shadow:3px 3px 10px gray;display:none">
        ...
        <div id="remoteClickToView" style="position:absolute;top:20px;left:0;width:100%;height:calc(100% - 30px);color:white;text-align:center;padding-top:20px;display:none" onclick="remotePlay(event)">表示するには、ここをクリックしてください。</div>
    </div>
    ...
    <script type="text/javascript" onunload="onUnLoad()">
        ...
        function displayControl(msg) {
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "コントロール" + '> ' + msg + '\r\n');
        ...
        function displayRemote(msg) {
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "リモート" + '> ' + msg + '\r\n');
        ...
        function xsend(event) {
            ...
            var outtext = Q('xouttext').value;
            if (outtext.length > 0) {
                chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "地元" + '> ' + outtext + '\r\n');
        ...
        function processMessage(data, transport) {
            if (typeof data == 'string') {
                ...
                switch (data.action) {
                    ...
                    case 'ctrl': {
                        if (data.value == 1) { displayControl("プッシュ通知として送信されます。"); }
                        else if (data.value == 2) { displayControl("プッシュ通知に失敗しました。"); }
        ...
        function startFileUpload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "アップロードする" + '> ' + file.name + ' (' + file.size + ' ' + "バイト" + ')\r\n');
        ...
        function startFileDownload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "ダウンロード" + '> ' + file.name + ' (' + file.size + ' ' + "バイト" + ')\r\n');
