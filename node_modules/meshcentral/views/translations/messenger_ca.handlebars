<!DOCTYPE html><html lang="ca" style="height:100%"><head>
...
<body style="font-family:Arial,Helvetica,sans-serif">
    <div id="xtop" style="position:absolute;left:0;right:0;top:0;height:38px;background-color:#036;color:#EEE;box-shadow:3px 3px 10px gray">
        ...
        <div style="position:absolute;background-color:#036;right:0;height:38px">
            <div id="saveButton" class="icon15 topButton" style="margin-right:4px" title="Guarda la conversa" onclick="saveChatSession()"></div>
            <div id="notifyButton" class="icon13 topButton" style="display:none" title="Activa la notificació del navegador" onclick="enableNotificationsButtonClick()"></div>
            <div id="fileButton" class="icon4 topButton" title="Comparteix un fitxer" style="display:none" onclick="fileButtonClick()"></div>
            <div id="camButton" class="icon2 topButton" title="Activa la càmera i el micròfon" style="display:none" onclick="camButtonClick()"></div>
            <div id="micButton" class="icon6 topButton" title="Activa el micròfon" style="display:none" onclick="micButtonClick()"></div>
            <div id="hangupButton" class="icon11 topRedButton" title="Penjar" style="display:none" onclick="hangUpButtonClick(1)"></div>
            <div id="recordIcon" class="deskareaicon" title="El servidor està gravant aquesta sessió" style="background-color:red;margin:6px;margin-top:7px;border-radius:12px;height:24px;width:24px;float:right;display:none"></div>
        </div>
    ...
    <div id="xbottom" style="position:absolute;left:0;right:0;bottom:0px;height:36px;background-color:#036;font-size:18px">
        <table style="width:100%">
            <tbody><tr>
                ...
                <td style="width:1px">
                    <input type="button" id="clearButton" value="Clar" style="box-sizing: border-box;float:right;font-size:18px" onclick="displayClear()">
                </td>
            </tr>
        </tbody></table>
    </div>
    <div id="remoteVideo" style="position:absolute;right:24px;top:45px;width:320px;height:calc(240px + 30px);background-color:gray;border-radius:12px 12px 12px 12px;box-shadow:3px 3px 10px gray;display:none">
        <div style="position:absolute;right:0;left:0;top:2.5px;text-align:center">Remot</div>
        ...
        <div id="remoteClickToView" style="position:absolute;top:20px;left:0;width:100%;height:calc(100% - 30px);color:white;text-align:center;padding-top:20px;display:none" onclick="remotePlay(event)">Feu clic aquí per veure'l.</div>
    </div>
    ...
    <script type="text/javascript" onunload="onUnLoad()">
        ...
        function displayRemote(msg) {
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Remot" + '> ' + msg + '\r\n');
        ...
        function disconnect() {
            ...
            QV('recordIcon', false);
            if (state > 0) { displayControl("Connexió tancada."); }
        ...
        function processMessage(data, transport) {
            if (typeof data == 'string') {
                ...
                switch (data.action) {
                    ...
                    case 'ctrl': {
                        if (data.value == 1) { displayControl("S'ha enviat com a notificació push."); }
                        else if (data.value == 2) { displayControl("La notificació push ha fallat."); }
        ...
        function fileSelect() {
            ...
            if (x.files.length > 10) {
                displayControl("Límit de 10 càrregues de fitxers al mateix temps.");
            } else {
        ...
        function startFileUpload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Carrega" + '> ' + file.name + ' (' + file.size + ' ' + "bytes" + ')\r\n');
        ...
        function startFileDownload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "descarregar" + '> ' + file.name + ' (' + file.size + ' ' + "bytes" + ')\r\n');
        ...
        function start() {
            ...
            if ((typeof args.id == 'string') && (args.id.length > 0)) {
                ...
                socket.onopen = function () { state = 1; displayControl("Esperant un altre usuari..."); }
                ...
                displayControl("Error: no s'ha especificat cap clau de connexió.");
            }
        }

        function saveChatSession() {
            saveAs(data2blob(chatTextSession), "Sessió de xat");
        }
