...
<body style="font-family:Arial,Helvetica,sans-serif">
    <div id="xtop" style="position:absolute;left:0;right:0;top:0;height:38px;background-color:#036;color:#EEE;box-shadow:3px 3px 10px gray">
        ...
        <div style="position:absolute;background-color:#036;right:0;height:38px">
            <div id="saveButton" class="icon15 topButton" style="margin-right:4px" title="Zapisz rozmowę" onclick="saveChatSession()"></div>
            <div id="notifyButton" class="icon13 topButton" style="display:none" title="Włącz powiadomienia w przeglądarce" onclick="enableNotificationsButtonClick()"></div>
            <div id="fileButton" class="icon4 topButton" title="Udostępnij plik" style="display:none" onclick="fileButtonClick()"></div>
            <div id="camButton" class="icon2 topButton" title="Aktywacja kamery i mikrofonu" style="display:none" onclick="camButtonClick()"></div>
            <div id="micButton" class="icon6 topButton" title="Aktywuj mikrofon" style="display:none" onclick="micButtonClick()"></div>
            <div id="hangupButton" class="icon11 topRedButton" title="Rozłącz" style="display:none" onclick="hangUpButtonClick(1)"></div>
            <div id="recordIcon" class="deskareaicon" title="Serwer nagrywa tę sesję" style="background-color:red;margin:6px;margin-top:7px;border-radius:12px;height:24px;width:24px;float:right;display:none"></div>
        </div>
    ...
    <div id="xbottom" style="position:absolute;left:0;right:0;bottom:0px;height:36px;background-color:#036;font-size:18px">
        <table style="width:100%">
            <tbody><tr>
                ...
                <td style="width:1px">
                    <input type="button" id="sendButton" value="Wyślij" style="box-sizing: border-box;float:right;font-size:18px" onclick="xsend(event)">
                </td>
                <td style="width:1px">
                    <input type="button" id="clearButton" value="Wyczyść" style="box-sizing: border-box;float:right;font-size:18px" onclick="displayClear()">
                </td>
            </tr>
        </tbody></table>
    </div>
    <div id="remoteVideo" style="position:absolute;right:24px;top:45px;width:320px;height:calc(240px + 30px);background-color:gray;border-radius:12px 12px 12px 12px;box-shadow:3px 3px 10px gray;display:none">
        <div style="position:absolute;right:0;left:0;top:2.5px;text-align:center">Zdalny</div>
        ...
        <div id="remoteClickToView" style="position:absolute;top:20px;left:0;width:100%;height:calc(100% - 30px);color:white;text-align:center;padding-top:20px;display:none" onclick="remotePlay(event)">Kliknij tutaj, aby wyświetlić.</div>
    </div>
    <div id="localVideo" style="position:absolute;right:24px;top:320px;width:160px;height:calc(120px + 30px);background-color:gray;border-radius:12px 12px 12px 12px;box-shadow:3px 3px 10px gray;display:none">
        <div style="position:absolute;right:0;left:0;top:2.5px;text-align:center">Lokalny</div>
    ...
    <script type="text/javascript" onunload="onUnLoad()">
        ...
        function displayRemote(msg) {
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Zdalny" + '> ' + msg + '\r\n');
        ...
        function xsend(event) {
            ...
            var outtext = Q('xouttext').value;
            if (outtext.length > 0) {
                chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Lokalny" + '> ' + outtext + '\r\n');
        ...
        function disconnect() {
            ...
            QV('recordIcon', false);
            if (state > 0) { displayControl("Połączenie zamknięte."); }
        ...
        function processMessage(data, transport) {
            if (typeof data == 'string') {
                ...
                switch (data.action) {
                    ...
                    case 'ctrl': {
                        if (data.value == 1) { displayControl("Wysłane jako powiadomienie push."); }
                        else if (data.value == 2) { displayControl("Powiadomienie Push nie powiodło się."); }
        ...
        function fileSelect() {
            ...
            if (x.files.length > 10) {
                displayControl("Limit wgrywania 10 plików w tym samym czasie.");
            } else {
        ...
        function startFileUpload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Prześlij" + '> ' + file.name + ' (' + file.size + ' ' + "Bajty" + ')\r\n');
        ...
        function startFileDownload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Pobierz" + '> ' + file.name + ' (' + file.size + ' ' + "Bajty" + ')\r\n');
        ...
        function start() {
            ...
            if ((typeof args.id == 'string') && (args.id.length > 0)) {
                ...
                socket.onopen = function () { state = 1; displayControl("Oczekiwanie na innego użytkownika..."); }
                ...
                displayControl("Błąd: Nie podano klucza połączenia.");
            }
        }

        function saveChatSession() {
            saveAs(data2blob(chatTextSession), "Sesja czatu");
        }
