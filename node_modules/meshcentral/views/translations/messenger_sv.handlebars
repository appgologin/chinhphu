<!DOCTYPE html><html lang="sv" style="height:100%"><head>
...
<body style="font-family:Arial,Helvetica,sans-serif">
    <div id="xtop" style="position:absolute;left:0;right:0;top:0;height:38px;background-color:#036;color:#EEE;box-shadow:3px 3px 10px gray">
        ...
        <div style="position:absolute;background-color:#036;right:0;height:38px">
            <div id="saveButton" class="icon15 topButton" style="margin-right:4px" title="Spara konversation" onclick="saveChatSession()"></div>
            <div id="notifyButton" class="icon13 topButton" style="display:none" title="Aktivera webbläsaravisering" onclick="enableNotificationsButtonClick()"></div>
            <div id="fileButton" class="icon4 topButton" title="Dela en fil" style="display:none" onclick="fileButtonClick()"></div>
            <div id="camButton" class="icon2 topButton" title="Aktivera kamera och mikrofon" style="display:none" onclick="camButtonClick()"></div>
            <div id="micButton" class="icon6 topButton" title="Aktivera mikrofon" style="display:none" onclick="micButtonClick()"></div>
            <div id="hangupButton" class="icon11 topRedButton" title="Lägg på" style="display:none" onclick="hangUpButtonClick(1)"></div>
            <div id="recordIcon" class="deskareaicon" title="Servern spelar in den här sessionen" style="background-color:red;margin:6px;margin-top:7px;border-radius:12px;height:24px;width:24px;float:right;display:none"></div>
        </div>
    ...
    <div id="xbottom" style="position:absolute;left:0;right:0;bottom:0px;height:36px;background-color:#036;font-size:18px">
        <table style="width:100%">
            <tbody><tr>
                ...
                <td style="width:1px">
                    <input type="button" id="sendButton" value="Skicka" style="box-sizing: border-box;float:right;font-size:18px" onclick="xsend(event)">
                </td>
                <td style="width:1px">
                    <input type="button" id="clearButton" value="Klar" style="box-sizing: border-box;float:right;font-size:18px" onclick="displayClear()">
                </td>
            </tr>
        </tbody></table>
    </div>
    <div id="remoteVideo" style="position:absolute;right:24px;top:45px;width:320px;height:calc(240px + 30px);background-color:gray;border-radius:12px 12px 12px 12px;box-shadow:3px 3px 10px gray;display:none">
        <div style="position:absolute;right:0;left:0;top:2.5px;text-align:center">Avlägsen</div>
        ...
        <div id="remoteClickToView" style="position:absolute;top:20px;left:0;width:100%;height:calc(100% - 30px);color:white;text-align:center;padding-top:20px;display:none" onclick="remotePlay(event)">Klicka här för att se.</div>
    </div>
    ...
    <script type="text/javascript" onunload="onUnLoad()">
        ...
        function displayControl(msg) {
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Kontrollera" + '> ' + msg + '\r\n');
        ...
        function displayRemote(msg) {
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Avlägsen" + '> ' + msg + '\r\n');
        ...
        function disconnect() {
            ...
            QV('recordIcon', false);
            if (state > 0) { displayControl("Anslutningen stängd."); }
        ...
        function processMessage(data, transport) {
            if (typeof data == 'string') {
                ...
                switch (data.action) {
                    ...
                    case 'ctrl': {
                        if (data.value == 1) { displayControl("Skickas som push-meddelande."); }
                        else if (data.value == 2) { displayControl("Push-avisering misslyckades."); }
        ...
        function fileSelect() {
            ...
            if (x.files.length > 10) {
                displayControl("Begränsning av 10 filuppladdningar samtidigt.");
            } else {
        ...
        function startFileUpload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Ladda upp" + '> ' + file.name + ' (' + file.size + ' ' + "byte" + ')\r\n');
        ...
        function startFileDownload(file) {
            ...
            chatTextSession += ((new Date()).toLocaleTimeString() + ' - ' + "Ladda ner" + '> ' + file.name + ' (' + file.size + ' ' + "byte" + ')\r\n');
        ...
        function start() {
            ...
            if ((typeof args.id == 'string') && (args.id.length > 0)) {
                ...
                socket.onopen = function () { state = 1; displayControl("Väntar på andra användare ..."); }
                ...
                displayControl("Fel: Ingen anslutningsnyckel specificerad.");
            }
        }
